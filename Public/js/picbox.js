...
(function ($) {
    ...
    var win = $(window)
        ...
        , browserIsCrap, middleX, middleY, imageX, imageY, currentSize, initialSize, imageDrag, timer, fitsOnScreen,
    ...
    */
    ...
    function keyDown(event) {
        ...
        return $.inArray(code, options.closeKeys) >= 0 ? close() : $.inArray(code, options.nextKeys) >= 0 ? next() : $.inArray(code, options.previousKeys) >= 0 ? previous() : false;
    }
    ...
    function showImage(noAnim) {
        ...
        var mw = win.width() - options.margins
            , mh = win.height() - options.margins
            , size = 1;
    ...
    function resizeImage(to, noAnim) {
        ...
        var width = preload.width * to
            ...
            dur = noAnim ? 0 : options.resizeDuration
            , fn = (0 == to) ? function () {
                $(image).hide()
            } : function () {};
        $(image).animate({
    ...
    $.tinyDrag = function (el, callback) {
        var mouseStart, elStart, moved, doc = $(document)
            , abs = Math.abs;
        ...
        function drag(e) {
            ...
            if (moved) {
                el.css({
                    left: elStart.x + (x - mouseStart.x)
                    , top: elStart.y + (y - mouseStart.y)
                });
            } else {
    ...
    $.fn.rotateImg = function (options) {
        ...
        this.each(function () {
            ...
            var m11 = Math.cos(rad) * options.scale
                , m12 = -1 * Math.sin(rad) * options.scale
                , m21 = Math.sin(rad) * options.scale
                , m22 = m11;
            if (iebrowser) {
